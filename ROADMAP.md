...
- **Content-pack depth is uneven.** The registry now has 19 skills and 22 packs, and the audit now distinguishes authored content from covered arithmetic DSL spec pools. `math.balance_equations.core`, `math.time_reading.core`, and `math.addition_memory.core` have been expanded to level-stage packs; arithmetic snake packs are considered covered when operation family + ranges + unlocks are complete. Remaining product risk is now explicit difficulty/learning-outcome metadata on language packs and `math.unit_conversions.core`.
